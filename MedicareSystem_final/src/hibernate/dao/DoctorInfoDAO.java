...
public class DoctorInfoDAO implements IDoctorInfoDAO {
	...
	public DoctorInfo findById(java.lang.String id) {
		...
		try {
			DoctorInfo instance = (DoctorInfo) getSession().get("hibernate.vo.userinfo.DoctorInfo", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<DoctorInfo> findByExample(DoctorInfo instance) {
		...
		try {
			List results = getSession().createCriteria("hibernate.vo.userinfo.DoctorInfo")
	...
	public List<DoctorInfo> findByProperty(String propertyName, Object value) {
		log.debug("finding DoctorInfo instance with property: " + propertyName + ", value: " + value);
		try {
			String queryPropertyString = "from DoctorInfo as model where model." + propertyName + "= ?";
	...
	public List<DoctorInfo> findAll() {
		...
		try {
			String queryAllString = "from DoctorInfo";
	...
	public DoctorInfo findById(Object id) {
		log.debug("getting DoctorInfo instance with id: " + id);
		try {
			DoctorInfo instance = (DoctorInfo) getSession().get("hibernate.vo.userinfo.DoctorInfo", (String)id);
			return instance;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
